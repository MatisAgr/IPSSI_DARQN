...
epsilon_decay = 0.995
gamma = 0.99
batch_size = 16  # Réduit de 32 pour CPU portable
memory_size = 25000  # Réduit de 50000 pour économiser la RAM
episodes = 500
...
def create_darqn_model():
    ...
    conv1 = layers.Conv2D(16, (8, 8), strides=(4, 4), activation='relu',  # Réduit de 32
                         padding='same', name='conv1')(input_layer)
    batch1 = layers.BatchNormalization()(conv1)
    
    conv2 = layers.Conv2D(32, (4, 4), strides=(2, 2), activation='relu',  # Réduit de 64
                         padding='same', name='conv2')(batch1)
    batch2 = layers.BatchNormalization()(conv2)
    
    conv3 = layers.Conv2D(64, (3, 3), strides=(1, 1), activation='relu',  # Réduit de 128
                         padding='same', name='conv3')(batch2)
    batch3 = layers.BatchNormalization()(conv3)
    
    # -------- ATTENTION MECHANISM (CBAM) --------
    attention = CBAM(ratio=16, kernel_size=5)(batch3)           # Ratio réduit de 8, kernel réduit de 7
    
    # -------- RECURRENT PROCESSING (TEMPORAL CONTEXT) --------
    flat = layers.Flatten()(attention)
    reshape_lstm = layers.Reshape((121, 64))(attention)         # Réduit de 128 pour correspondre à conv3
    lstm1 = layers.LSTM(128, return_sequences=True, activation='relu',    # Réduit de 256
                       name='lstm1', dropout=0.2)(reshape_lstm)
    lstm2 = layers.LSTM(64, return_sequences=False, activation='relu',    # Réduit de 128
    ...
    dense1 = layers.Dense(256, activation='relu', name='dense1')(merge)   # Réduit de 512
    dropout1 = layers.Dropout(0.3)(dense1)
    
    dense2 = layers.Dense(128, activation='relu', name='dense2')(dropout1) # Réduit de 256
    dropout2 = layers.Dropout(0.2)(dense2)
    
    # -------- DUELING ARCHITECTURE --------
    # Value Stream: estimates state value V(s)
    value_dense = layers.Dense(64, activation='relu',                      # Réduit de 128
    ...
    advantage_dense = layers.Dense(64, activation='relu',                  # Réduit de 128
                                  name='advantage_dense')(dropout2)
    advantage_output = layers.Dense(action_space, name='advantage')(
        advantage_dense)
